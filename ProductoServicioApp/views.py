...
from .models import Abastecimiento, ProductoExcel, ProductoServicio
...
def subir_excel_producto(request, pk):
    ...
    if request.method != "POST":
        messages.error(request, "Debes seleccionar un archivo Excel para este producto.")
        return redirect("productoyservicioapp:lista_productos")

    archivos = request.FILES.getlist("archivos_excel")
    if not archivos:
        messages.error(request, "Selecciona al menos un archivo .xlsx.")
        return redirect("productoyservicioapp:lista_productos")
    ...
    if procesados:
        messages.success(
            request,
            f"Se cargaron {procesados} archivo(s) Excel para '{producto.produ_nom}' y se actualizaron sus datos contables.",
        )

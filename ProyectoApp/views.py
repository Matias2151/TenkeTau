...
def lista_proyectos(request):
    ...
    # opcional: select_related para no pegar muchos queries
...
def crear_proyecto(request):
    ...
    if request.method != "POST":
        return JsonResponse(
            {"success": False, "errors": {"_all_": ["Método no permitido."]}},
            status=405,
        )
    ...
    # 🔁 MODIFICADO: asignar cliente manualmente desde el <select name="cliente">
    proyecto = form.save(commit=False)

    cliente_id = request.POST.get("cliente")  # 👈 viene del select del template
    if cliente_id:
        try:
            # Ajusta el nombre del campo FK si no se llama "cliente"
            proyecto.cliente = EmpresaPersona.objects.get(pk=cliente_id)
        except EmpresaPersona.DoesNotExist:
            if es_ajax(request):
                return JsonResponse(
                    {"success": False, "errors": {"cliente": ["El cliente seleccionado no existe."]}},
                    status=400,
                )
    ...
    if es_ajax(request):
        return JsonResponse({
            ...
            "created": {
                ...
                # Fechas en formato ISO (YYYY-MM-DD) para que JS las convierta a dd/mm/aaaa
                "fecha_sol": proyecto.proye_fecha_sol.strftime("%Y-%m-%d") if getattr(proyecto, "proye_fecha_sol", None) else "",
                "fecha_ter": proyecto.proye_fecha_ter.strftime("%Y-%m-%d") if getattr(proyecto, "proye_fecha_ter", None) else "",
...
def editar_proyecto(request, pk):
    ...
    # 🔁 MODIFICADO: actualizar también el cliente desde el POST
    proyecto = form.save(commit=False)

    cliente_id = request.POST.get("cliente")
    if cliente_id:
        try:
            proyecto.emppe_nom = EmpresaPersona.objects.get(pk=cliente_id)
        except EmpresaPersona.DoesNotExist:
            if es_ajax(request):
                return JsonResponse(
                    {"success": False, "errors": {"cliente": ["El cliente seleccionado no existe."]}},
        ...
        # Si quieres permitir dejar el proyecto sin cliente, descomenta esta línea:

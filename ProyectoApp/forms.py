...
class ProyectoForm(forms.ModelForm):
    """Formulario para creación y edición de proyectos (sin FK directa a cliente en el form)."""
    ...
    proye_estado = forms.ChoiceField(
        ...
        error_messages={
            "required": "Debes seleccionar un estado para el proyecto.",
        },
    )
    ...
    def clean(self):
        ...
        if fecha_sol and fecha_ter and fecha_ter < fecha_sol:
            self.add_error(
                "proye_fecha_ter",
                "La fecha de término no puede ser anterior a la fecha de solicitud."
            )
        ...
        if fecha_ter and fecha_ter > hoy.replace(year=hoy.year + 5):
            self.add_error(
                "proye_fecha_ter",
                "La fecha de término no puede superar 5 años desde hoy."
            )
